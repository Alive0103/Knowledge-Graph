#!/bin/bash
# Restart only --vector-llm with 1 worker and 5s rate limit
# Resumes from checkpoint 237/444
set -e
...
LOG="D:/work/毕设/知识图谱/Knowledge-Graph/retry/output/kb_t075_bge_eval.log"

echo "[$(date -Iseconds)] Restarting --vector-llm (1 worker, 5s interval, resume from ~237/444)" >> "$LOG"
cd "$WORKDIR" && "$PYTHON" "$SCRIPT" --vector-llm
echo "[$(date -Iseconds)] Done: --vector-llm" >> "$LOG"
echo "[$(date -Iseconds)] ALL 5 MODES COMPLETE (BGE-M3)" >> "$LOG"
