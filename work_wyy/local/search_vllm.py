...
try:
    ...
    ES_INDEX_NAME = 'data2'
    TRAINLOG_DIR = None
    ZHIPUAI_API_KEY = os.getenv("KG_ZHIPU_API_KEY") or os.getenv("ZHIPUAI_API_KEY")
    if not ZHIPUAI_API_KEY:
        raise RuntimeError("Missing KG_ZHIPU_API_KEY or ZHIPUAI_API_KEY environment variable.")
    VECTOR_SEARCH_TOP_K = 30
    LLM_RERANK_TOP_K = 30
    WORK_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    from es_client import es
ES_INDEX_NAME = os.getenv("KG_ES_INDEX_NAME", ES_INDEX_NAME or 'data2')
...
GLM_THINKING_ENABLED = os.getenv("KG_EVAL_GLM_THINKING", "1").strip().lower() not in {"0", "false", "no", "off"}
...
def create_chat_completion(messages, model_name="glm-5.1"):
    ...
    if model_name.startswith("glm-5"):
        request_kwargs["thinking"] = {"type": "enabled" if GLM_THINKING_ENABLED else "disabled"}
    ...
    if model_name.startswith("glm-5") and GLM_THINKING_ENABLED:
        ...
        if not response_text:
            logger.warning("GLM-5 returned empty final content with thinking enabled; retrying without thinking.")
...
def generate_prompt_and_sort_with_description(mention, results, use_vector_results=True):
    ...
    try:
        response = create_chat_completion([{"role": "user", "content": prompt}])
        response_text = _extract_message_text(response.choices[0].message)
