...
class PrerequisitesChecker:
    ...
    def __init__(self):
        # 获取配置文件中的路径
        try:
            from config import BASE_MODEL_PATH, DATA_DIR, MODEL_DIR
            ...
            self.base_model_path = self.model_dir / "chinese-roberta-wwm-ext-large"
        self.errors = []
        self.warnings = []
    ...
    def check_model_exists(self):
        ...
        if model_path.exists():
            ...
            all_exist = all((model_path / f).exists() for f in required_files) and any((model_path / f).exists() for f in weight_files)
