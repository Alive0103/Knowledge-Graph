...
try:
    from config import BASE_MODEL_PATH as CONFIG_BASE_MODEL_PATH, FINETUNED_MODEL_PATH as CONFIG_FINETUNED_MODEL_PATH
except ImportError:
    CONFIG_BASE_MODEL_PATH = None
    CONFIG_FINETUNED_MODEL_PATH = None
...
# 优先使用微调后的模型
NER_MODEL_PATH = (
    os.getenv("KG_FINETUNED_MODEL_PATH")
    or os.getenv("KG_VECTOR_MODEL_PATH")
    or CONFIG_FINETUNED_MODEL_PATH
    or os.path.join(_parent_dir, 'model', 'ner_finetuned')
)
BASE_MODEL_PATH = (
    os.getenv("KG_WORK_WYY_BASE_MODEL_PATH")
    or CONFIG_BASE_MODEL_PATH
    or os.path.join(_parent_dir, 'model', 'chinese-roberta-wwm-ext-large')
)
