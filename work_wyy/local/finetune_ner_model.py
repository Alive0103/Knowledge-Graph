...
try:
    ...
    BASE_MODEL_PATH = os.path.join(os.path.dirname(_script_dir), 'model', 'chinese-roberta-wwm-ext-large')
    FINETUNED_MODEL_PATH = os.path.join(os.path.dirname(_script_dir), 'model', 'ner_finetuned')
    TRAINLOG_DIR = os.path.join(os.path.dirname(_script_dir), 'trainlog')
    TRAINING_CONFIG = {
        'max_length': 512,
        'batch_size': 8,
        'learning_rate': 2e-5,
...
log_file = os.path.join(TRAINLOG_DIR, 'ner_finetune.log')
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    # 数据目录（从多个目录加载数据）
    # 使用配置文件中的数据目录
    base_data_dir = get_data_base_dir()
    
    # 从所有数据目录加载数据（使用data_loader模块）
    logger.info("从多个数据目录加载数据...")
    logger.info(f"数据目录: {base_data_dir}")
