...
import torch
import numpy as np
from transformers import AutoModel, AutoModelForTokenClassification, AutoTokenizer, BertTokenizerFast
import logging
...
def _load_local_tokenizer(model_path):
    """优先加载本地 fast tokenizer，必要时降级到显式 tokenizer.json。"""
    try:
        ...
        logger.warning(f"Fast tokenizer load failed for {model_path}: {fast_exc}")
    ...
    if os.path.exists(tokenizer_json):
        try:
            ...
            logger.warning(f"BertTokenizerFast(tokenizer.json) load failed for {model_path}: {json_exc}")
...
def load_vector_model(model_path=None, use_finetuned=True):
    ...
    env_model_path = os.getenv("KG_VECTOR_MODEL_PATH") or os.getenv("KG_FINETUNED_MODEL_PATH")
    if model_path is None and env_model_path:
        resolved_env_model_path = os.path.abspath(env_model_path)
        if os.path.exists(resolved_env_model_path):
            model_path = resolved_env_model_path
            logger.info(f"Using vector model from environment override: {model_path}")
        else:
            logger.warning(f"Configured KG_VECTOR_MODEL_PATH does not exist: {resolved_env_model_path}")
    ...
    try:
        # 加载tokenizer
        _vector_tokenizer = _load_local_tokenizer(model_path)
        ...
        try:
            ...
            _vector_model = getattr(ner_model, "bert", None)
            if _vector_model is None:
                _vector_model = getattr(ner_model, "base_model", None)
            if _vector_model is None:
                raise AttributeError("token classification model has no encoder attribute")
            logger.info(f"从NER模型中提取encoder用于向量生成: {model_path}")
        except:
            # 如果不是NER模型，直接加载BertModel
            _vector_model = AutoModel.from_pretrained(
                model_path,
                local_files_only=True,
            )
...
def generate_vector(text, model_path=None, use_finetuned=True, target_dim=1024):
    ...
    try:
        ...
        # 生成向量
        _use_mean_pooling = os.getenv("KG_VECTOR_POOLING", "cls").lower() == "mean"
        with torch.no_grad():
            outputs = _vector_model(**inputs)
            if _use_mean_pooling:
                hidden = outputs.last_hidden_state
                mask = inputs["attention_mask"].unsqueeze(-1).expand(hidden.size()).float()
                vector = ((hidden * mask).sum(1) / mask.sum(1).clamp(min=1e-9)).squeeze().cpu().numpy()
            else:
                # 使用[CLS] token的向量
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if _use_mean_pooling:
                    hidden = outputs.last_hidden_state
                    mask = inputs["attention_mask"].unsqueeze(-1).expand(hidden.size()).float()
                    batch_vectors = ((hidden * mask).sum(1) / mask.sum(1).clamp(min=1e-9)).cpu().numpy()
                else:
